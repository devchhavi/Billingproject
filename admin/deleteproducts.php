...
require_once('../include/db.php');
  $p_name = $_POST['p_name'];
 $ResultAllsss = mysqli_query($link, "SELECT * FROM `purchase_product_list` WHERE purchase_list_id='$p_name'");
 ...
 $query = "DELETE FROM `purchase_product_list` WHERE purchase_list_id=$p_name";
 $result = mysqli_query($link, $query);


 

 //mysqli_query($link, "UPDATE `product` SET `Available`=Available-$quantityy,`Purchase`=Purchase-$quantityy WHERE Product_Code='$productcode'");
